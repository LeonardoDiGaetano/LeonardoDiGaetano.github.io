...
<style>
  ...
  /* Container for your publication list */
  #content-container {
    position: relative;
    z-index: 1;
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function update() {
      ...
      // 1) Fill triangles between mutually‑connected nodes
      ...
      // 2) Update positions (repulsion + friction)
      nodes.forEach(n => {
        if (mouse.x !== null) {
      ...
      // 3) Draw edges, highlight lines, and nodes
      for (let i = 0; i < NODE_COUNT; i++) {
        const n = nodes[i];
